...
We perform a breadth-first state space exploration during which we check soundness and safeness~\cite{corradiniClassificationBPMNCollaborations2018} \textit{on the fly} to uncover control flow errors.
...
In general, our analysis is instantaneous for nearly all models since most have less than 1000 states according to~\cite{fahlandAnalysisDemandInstantaneous2011}.
...
We are unaware of other tools that visualize errors directly and allow for interactions, such as stopping/resuming and restarting.
...
The BPMN Analyzer is incorporates many findings from our previous work~\cite{krauterHigherorderTransformationApproach2024} while focusing on instantaneous and understandable error detection, as described in the previous section.
